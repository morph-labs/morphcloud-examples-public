    ...
    async def execute(self, code: str, kernel_id: str = None) -> dict:
        ...
        while True:
            ...
            try:
                ...
                console.print(f"[dim]Received message: {msg_type} (parent: {parent_msg_id})[/dim]")
...
class JupyterNotebookClient:
    ...
    async def list_notebooks(self, path: str = ""):
        ...
        async with httpx.AsyncClient() as client:
            ...
            notebooks = [item for item in result.get('content', []) 
                        if item.get('type') == 'notebook']
            
            if notebooks:
                console.print("[green]Found notebooks:[/green]")
                for nb in notebooks:
                    console.print(f"- {nb['name']} (Last modified: {nb['last_modified']})")
            else:
    ...
    async def add_cell(self, notebook_path: str, cell_content: str, cell_type: str = "code", index: int = None):
        """Add a cell to a notebook"""
        console.print(f"[yellow]Adding {cell_type} cell to notebook: {notebook_path}[/yellow]")
        ...
        if index is not None:
            ...
            console.print(f"[green]Cell appended at index {len(notebook['cells'])-1}[/green]")
    ...
    async def execute_cell(self, notebook_path: str, cell_index: int, kernel_id: str = None):
        ...
        if cell_index < 0 or cell_index >= len(notebook["cells"]):
            raise ValueError(f"Cell index {cell_index} out of range for notebook with {len(notebook['cells'])} cells")
...
"""
        ...
        snapshot = await snapshot.asetup(f"""
        ...
        with self.instance.ssh() as ssh:
            ...
            try:
                ...
                try:
                    ...
                    console.print(f"[red]Error: Remote path '{remote_path}' not found[/red]")
                    raise
            finally:
                sftp.close()
        
        console.print(f"[green]Download completed: {remote_path} -> {local_path}[/green]")
        return True
    ...
    async def list_remote_files(self, remote_path):
        ...
        with self.instance.ssh() as ssh:
            ...
            try:
                ...
                for item in items:
                    ...
                    mtime = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(item.st_mtime))
        ...
        if files:
            ...
            for file in files:
                ...
                console.print(f"{icon} {file['name']} ({file['size']} bytes, {file['modified']})")
        else:
        ...
        with self.instance.ssh() as ssh:
            ...
            try:
                ...
                try:
                    ...
                    if stat.S_ISDIR(attr.st_mode):
                        if recursive:
                            ...
                            try:
                                ...
                                console.print(f"[red]Error: Directory '{remote_path}' is not empty. Use recursive=True to remove.[/red]")
                                return False
                    else:
                        # It's a file - remove it
                    ...
                    console.print(f"[yellow]Warning: Remote path '{remote_path}' not found[/yellow]")
                    return False
            finally:
                sftp.close()
