...
def run_ssh_command(instance, command, sudo=False, print_output=True, timeout=None):
    ...
    if ssh_timeout is None:
        ...
        execution_thread = threading.Thread(target=lambda: globals().update({'result': execute_command(), 'has_result': True}))
        execution_thread.daemon = True  # Allow Python to exit even if this thread is running
    ...
    if result and result.exit_code != 0 and result.exit_code != 124:  # Don't show error for timeouts
    ...
    for snapshot in existing_snapshots:
        if (snapshot.status == "ready" and
            ...
            if node_required and snapshot.metadata.get("has_node") != "true":
                print(f"Snapshot {snapshot.id} matches but doesn't have Node.js installed. Continuing search...")
                continue
...
def fetch_mcp_packages():
    ...
    try:
        response = requests.get("https://raw.githubusercontent.com/michaellatman/mcp-get/refs/heads/main/packages/package-list.json")
...
def select_mcp_package(packages):
    ...
    while True:
        try:
            selection = input("Select an MCP server by number (or press Enter for Brave Search): ")
            ...
            if not path.endswith('/'):
                ...
                if dir_path:
                    for name in readme_names:
                        urls.append(f"https://raw.githubusercontent.com/{username}/{repo}/{branch}/{dir_path}/{name}")
                
            # Check in the path directly (if it's a directory)
            for name in readme_names:
                urls.append(f"https://raw.githubusercontent.com/{username}/{repo}/{branch}/{path}/{name}")
        
        # Last resort: try repo root
        for name in readme_names:
            urls.append(f"https://raw.githubusercontent.com/{username}/{repo}/{branch}/{name}")
    ...
    for block in inline_blocks:
        ...
        if block_content not in json_texts:
            ...
            try:
                ...
                if block_content.count('{') == block_content.count('}') and '"' in block_content:
                    json_blocks.append({
                        "json": None,
                        "text": block_content,
                        "error": "Invalid JSON syntax"
...
def find_mcp_config_from_readme(package):
    ...
    if not github_info:
        print("Could not parse GitHub URL. Make sure it's a valid GitHub repository URL.")
        return None
...
def setup_supergateway_multi(instance, command_info, service_name, port=3000, enable_cors=False):
    """
...
def setup_supergateway(instance, command_info, port=3000, enable_cors=False):
    ...
    """
    return setup_supergateway_multi(instance, command_info, "mcp-sse", port, enable_cors)
...
def detect_existing_services(instance):
    ...
    result = run_ssh_command(instance, "systemctl list-units --type=service --state=running | grep mcp-sse", print_output=False)
        ...
        for line in port_lines:
            if 'node' in line or 'supergateway' in line:
                ...
                if port_match and pid_match:
                    ...
                    cmd_result = run_ssh_command(instance, f"ps -p {pid} -o command=", print_output=False)
...
def display_connection_info(instance, server_configs, custom_urls=None):
    ...
    for i, (server_config, custom_url) in enumerate(zip(server_configs, custom_urls)):
        ...
        service_name = custom_url if custom_url else f"remote-server-{server_config['name'].split('/')[-1]}"
        ...
        if sse_service:
            ...
            # SSE version
            unified_configs_json["mcpServers"][f"{package_name}-sse"] = {
                "url": sse_url
            }
            ...
            print(f"Could not find SSE service URL for {server_config['name']}. Please check the VM manually.")
...
def prompt_for_env_vars(env_vars):
    ...
    for key, value in env_vars.items():
        ...
        if ("key" in key.lower() or "token" in key.lower() or "secret" in key.lower()) and value != "":
        ...
        while True:
            ...
            try:
                ...
                if option == 1:
                    ...
                    key_idx = int(input("Enter number of variable to remove: ").strip()) - 1
        ...
        while True:
            ...
            try:
                ...
                if option == 1:
                    ...
                    arg_idx = int(input("Enter number of argument to update: ").strip()) - 1
                    if 0 <= arg_idx < len(updated_args):
                        new_value = input(f"Enter new value for '{updated_args[arg_idx]}': ").strip()
                    ...
                    arg_idx = int(input("Enter number of argument to remove: ").strip()) - 1
                    if 0 <= arg_idx < len(updated_args):
                    ...
                    from_idx = int(input("Enter number of argument to move: ").strip()) - 1
                    to_idx = int(input("Enter target position: ").strip()) - 1
                    
                    if 0 <= from_idx < len(updated_args) and 0 <= to_idx < len(updated_args):
...
def check_nodejs_installation(instance, update_metadata=True):
    ...
    if result.exit_code == 0:
        ...
        install_result = run_ssh_command(instance, "apt-get update -y && apt-get install -y nodejs npm", sudo=True)
        
        if install_result.exit_code == 0:
            # Verify installation
            verify_result = run_ssh_command(instance, "node --version && npm --version", print_output=False)
...
def setup_single_server(instance, args, existing_services=None, server_count=0):
    ...
    if args.config:
        config_path = args.config[0] if isinstance(args.config, list) and len(args.config) > 0 else args.config
        print(f"\nLoading configuration from file: {config_path}")
        server_config = load_config_from_file(config_path)
        if server_config:
            print("Configuration loaded successfully")
        else:
            print("Failed to load configuration from file. Continuing with selection...")
    ...
    if not setup_supergateway_multi(instance, server_config, service_name, port=port, enable_cors=args.all_cors):
    ...
    http_service_name = generate_unique_service_name(http_service_name, {
...
def update_snapshot_metadata(snapshot, server_configs, service_names, instance=None):
    ...
    new_metadata["description"] = f"Multi-MCP Server with {len(server_configs)} services"
    
    # If instance is provided, check and add Node.js information
    if instance:
        # Check if Node.js is installed
        result = run_ssh_command(instance, "node --version && npm --version", print_output=False)
        if result.exit_code == 0 and result.stdout.strip():
            ...
            if len(versions) >= 2:
                ...
                print(f"Added Node.js ({node_version}) and npm ({npm_version}) information to snapshot metadata")
    else:
...
def main():
    ...
    try:
        ...
        import argparse
        parser = argparse.ArgumentParser(description="Setup multiple MCP servers on MorphCloud")
        ...
        parser.add_argument("--disk-size", type=int, default=4096, help="Disk size in MB")
        parser.add_argument("--node-required", action="store_true", default=True, help="Require Node.js to be installed")
        
        # Connection arguments
        parser.add_argument("--instance-id", help="Connect to an existing instance instead of creating a new one")
        
        # Server configuration arguments
        parser.add_argument("--config", nargs='+', help="Path to JSON config file(s)")
        
        # Multi-server arguments
        parser.add_argument("--multi", action="store_true", help="Set up multiple servers")
        parser.add_argument("--count", type=int, help="Number of servers to set up (when using --multi)")
        parser.add_argument("--base-port", type=int, default=3000, help="Starting port number for port allocation")
        parser.add_argument("--all-cors", action="store_true", help="Enable CORS for all servers")
        ...
        if args.instance_id:
            ...
            snapshot = get_or_create_snapshot(client, VCPUS, MEMORY, DISK_SIZE, node_required=args.node_required)
        ...
        for i in range(server_count):
            ...
            if server_config and service_name:
                ...
                print(f"Successfully set up server {len(all_server_configs)} of {server_count}")
        ...
        if all_server_configs:
            ...
            update_snapshot_metadata(final_snapshot, all_server_configs, all_service_names, instance)
            print(f"Final snapshot created: {final_snapshot.id}")
            
            # Display connection information for all servers
            display_connection_info(instance, all_server_configs, custom_urls=all_service_names)
            
            print(f"\nInstance ID: {instance.id}")
            print(f"Your MCP server{'s are' if len(all_server_configs) > 1 else ' is'} now running!")
            print(f"To stop the VM later, run: morphcloud instance stop {instance.id}")
            print(f"To restart from snapshot, run: morphcloud instance start {final_snapshot.id}")
        else:
