...
from morphcloud.api import MorphCloudClient # Import MorphCloudClient - needed for user setup function
...
def write_results_to_csv(book_title: str, result_data: dict, csv_file: str = "amazon_books_results.csv"):
...
async def process_books_distributed(books: list[str], max_parallel: int = 3, logged_in_snapshot_id="amazon-logged-in"): # Pass snapshot ID
    ...
    with open("amazon_books_results.csv", 'w', newline='', encoding='utf-8') as f:
        writer = csv.DictWriter(f, fieldnames=['book_title', 'timestamp', 'book_url', 'success'])
        writer.writeheader()
    ...
    async def process_book(worker_id: int):
        ...
        while not work_queue.empty():
            ...
            try:
                console.print(f"[blue]Worker {worker_id}: Starting work on book {books_processed + 1}/{total_books}: '{book}'[/blue]")

                # Use MorphBrowser.create with the logged-in snapshot
                async with await MorphBrowser.create(snapshot_id=logged_in_snapshot_id) as browser_instance:
                    browser = Browser(config=BrowserConfig(
                        cdp_url=browser_instance.cdp_url # Get CDP URL from MorphBrowser
                    ))

                    console.print(f"[yellow]Worker {worker_id}: Processing '{book}'[/yellow]")
                    agent = Agent(
                        browser=browser,
                    ...
                    history = await agent.run() # history is now assigned if agent.run() is successful
                    urls = history.urls()
                    last_url = urls[-1] if len(urls) >= 1 else (urls[0] if urls else None)
                    book_result = {
                    ...
                    books_processed += 1
                    console.print(f"[green]Worker {worker_id}: Processed '{book}' ({books_processed}/{total_books} completed)[/green]")
                    console.print(f"[green]Worker {worker_id}: Results for '{book}' saved to CSV[/green]")
                ...
                if history: # Check if history is defined before accessing it
                    error_details.update({
                        ...
                        "action_results": [result.model_dump() for result in history.action_results()],
                        ...
                        "model_thoughts": [thought.model_dump() for thought in history.model_thoughts()],
                    })
                book_error_result = {
                    "timestamp": datetime.now().isoformat(),
                    **error_details
                ...
                console.print(f"[blue]Worker {worker_id}: Finished with '{book}', {total_books - books_processed} books remaining[/blue]")
    ...
    console.print(f"[bold]Starting processing of {total_books} books using {max_parallel} workers[/bold]")
    ...
    console.print(f"[bold green]Completed processing {books_processed}/{total_books} books[/bold green]")
    console.print(f"[bold green]Results saved to amazon_books_results.csv and amazon_books_results.json[/bold green]")
...
async def setup_browser_for_amazon_login():
    ...
    completion_prompt = "close the tab after you have logged in and the Amazon homepage is loaded..."
...
async def main():
    ...
    if perform_user_setup:
        console.print("[bold yellow]Starting user setup for Amazon login...[/bold yellow]")
        logged_in_snapshot_id = await setup_browser_for_amazon_login() # Run user setup
        console.print(f"[bold green]User setup complete. Snapshot ID for logged-in state: {logged_in_snapshot_id}[/bold green]")
    else:
        console.print(f"[bold blue]Using existing snapshot ID for logged-in state: {logged_in_snapshot_id}[/bold blue]")
